...
class Spikeball: Enemy{
    ...
    convenience init(position: CGPoint, nodeName: String, scalingFactor: CGFloat?) {
        ...
        let physicsBody = SKPhysicsBody(texture: spikeballTexture, size: spikeballTexture.size())
        ...
        let animationsDict = getAnimationsDict(position: position)
        
        
        let animationComponent = BasicAnimationComponent(animationsDict: animationsDict)
        addComponent(animationComponent)
        
        animationComponent.runAnimation(withAnimationNameOf: "turnAnimation", andWithAnimationKeyOf: "turnAnimation")
        
        let intelligenceComponent = IntelligenceComponent(states: [
            HorizontalMoveState(enemyEntity: self),
            VerticalMoveState(enemyEntity: self),
            PathMoveState(enemyEntity: self)
            ])
        addComponent(intelligenceComponent)
        intelligenceComponent.stateMachine?.enter(HorizontalMoveState.self)
    ...
    func getAnimationsDict(position: CGPoint) -> [String: SKAction] {

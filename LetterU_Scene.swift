...
class LetterU_Scene: BaseScene{
    
    convenience init(size: CGSize, reloadData: ReloadData?){
        
        self.init(sksFileName: "FireScene", size: size, reloadData: reloadData)
    }
        ...
        if let nodeName = node.name,nodeName.contains("Enemy/"){
            ...
            if nodeName.contains("Animal/"){
                ...
                if nodeName.contains("Snake"){
                    ...
                    let snake = Animal(animalType: .Snake, position: position, nodeName: "snake\(position)", scalingFactor: 0.50)
